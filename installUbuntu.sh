...
if [ $USERNAME = "root" ] ; then
  ...
  source $USERHOME/.bashrc && [ -s "$NVM_DIR/nvm.sh" ] && \. "$NVM_DIR/nvm.sh" && [ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion" && cd $pwd && nvm install "$VER" && nvm use && npm install --build-from-source --python=/usr/bin/python3
...
echo -e "Run manually with: \x1b[92m./localdev_bootstrap.sh\x1b[0m" && \
echo -e "Or restart with: \x1b[92m./localdev_bootstrap.sh restart\x1b[0m" && \
